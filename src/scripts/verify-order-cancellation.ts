...
async function runOrderCancellationVerification() {
  ...
  try {
    ...
    let testGame = await Game.findOne({ name: "Cancellation Test Game" });
    if (!testGame) {
    ...
    let testPackage = await Package.findOne({ name: "100 Coins Package" });
    if (!testPackage) {
    ...
    const createTestOrder = async (orderStatus: "pending" | "processing" | "completed" | "cancelled") => {
      return Order.create({
        user: buyer._id,
        game: testGame._id,
    ...
    async function sendCancelRequest(orderId: string, token?: string) {
    ...
    if (res5.status === 200 && res5.body.success) {
      ...
      await Order.deleteOne({ _id: orderPendingAdmin._id });
    } else {
    ...
    if (res6.status === 403) {
      ...
      await Order.deleteOne({ _id: orderPendingOther._id });
    } else {
      console.log("🔴 FAILURE: Expected status 403.");
    }

    // Cleanup
    await Order.deleteMany({ _id: { $in: [orderPending._id, orderCompleted._id, orderCancelled._id] } });
