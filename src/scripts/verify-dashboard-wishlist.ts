...
async function main() {
  ...
  let game1 = await Game.findOne({ slug: "free-fire-max" });
  if (!game1) {
  ...
  let game2 = await Game.findOne({ slug: "pubg-mobile" });
  if (!game2) {
  ...
  // Verify isolation
  const containsUserBItem = wishlistA.some(
    (item: any) => item.game?._id === game2._id.toString() || item.game === game2._id.toString()
  );
  ...
  const userBItems = await Wishlist.find({ user: userB._id });
